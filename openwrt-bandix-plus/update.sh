...
RUST_BANDIX_PLUS_VERSION="$(sed -n 's|RUST_BANDIX_PLUS_VERSION:=||p' "$CURDIR/Makefile")"
[ "$RUST_BANDIX_PLUS_VERSION" != "$LATEST_VERSION" ] || exit 0
...
for (( i=0; i<$(jq -r '.assets | length' <<< "$LATEST_INFO"); i++ )); do
	name=$(jq -r '.assets['$i'].name | select(contains("bandix-plus"))' <<< "$LATEST_INFO")
	[ -n "$name" ] || continue
	...
	arch="$(awk -F'-' '{ printf "%s-%s", $1, $NF }' <<< "$(sed "s|bandix-plus-$LATEST_VERSION-||;s|\.tar\..*||" <<< "$name")")"
...
sed -i "s|\(RUST_BANDIX_PLUS_VERSION:=\).*|\1$LATEST_VERSION|" "$CURDIR/Makefile"